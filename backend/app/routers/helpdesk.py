...
async def seed(db: AsyncSession):
    ...
    if c.scalar() == 0:
        for cat in DEFAULT_CATEGORIES:
            ...
            await db.execute(text("INSERT INTO ticket_categories (id,name,description,color,icon,active,created_at) VALUES (CAST(:id AS uuid),:name,'',:color,:icon,true,NOW())"), {**cat,"id":cat_id})
...
async def get_categories(db:AsyncSession=Depends(get_db)):
    ...
    for cat in r.fetchall():
        ...
        subs=await db.execute(text("SELECT c.*,g.name as group_name FROM ticket_categories c LEFT JOIN helpdesk_groups g ON c.group_id=g.id WHERE c.parent_id=CAST(:pid AS uuid) AND c.active=true ORDER BY c.name"),{"pid":str(cd["id"])})
...
async def get_groups(db:AsyncSession=Depends(get_db)):
    ...
    for g in r.fetchall():
        gd=dict(g._mapping)
        m=await db.execute(text("SELECT * FROM helpdesk_group_members WHERE group_id=CAST(:gid AS uuid) ORDER BY user_name"),{"gid":str(gd["id"])})
...
async def add_group_member(gid:str,data:dict,db:AsyncSession=Depends(get_db)):
    ...
    if data.get("is_responsible"):
        await db.execute(text("UPDATE helpdesk_groups SET responsible_email=:email,responsible_name=:name WHERE id=CAST(:id AS uuid)"),
