...
type DataScope = 'own' | 'team' | 'company'
interface TasksPermCtx { canEdit: boolean; dataScope: DataScope }
export const TasksPermContext = createContext<TasksPermCtx>({ canEdit: false, dataScope: 'own' })
export const useTasksPerm = () => useContext(TasksPermContext)

const NAV_ITEMS = [
  { href: '/task-manager', label: 'My Tasks', icon: '✅' },
]
...
export default function TasksLayout({ children }: { children: React.ReactNode }) {
    ...
    <aside style={{ width: '220px', minHeight: '100vh', background: '#156082', position: 'fixed', left: 0, top: 0, zIndex: 100, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '18px 16px 14px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <span style={{ color: 'white', fontSize: '12px', fontWeight: '800', letterSpacing: '0.05em' }}>TASK MANAGER</span>
        </div>
      </div>
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <main style={{ marginLeft: '220px', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F5F7FA' }}>
        <div style={{ textAlign: 'center', color: '#94A3B8' }}>
          ...
          <p style={{ fontSize: '13px', margin: '0 0 20px' }}>You don't have permission to access the Task Manager module.</p>
  ...
  return (
    <TasksPermContext.Provider value={{ canEdit: permLevel === 'edit', dataScope }}>
      <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Montserrat, sans-serif' }}>
        {sidebar}
