...
const NAV = [
  { href: '/testing/test-plans',        icon: '📋', label: 'Test Plans' },
  { href: '/testing/test-campaigns',    icon: '🧪', label: 'Test Campaigns' },
  { href: '/testing/remediation-plans', icon: '🛠️', label: 'Remediation Plans' },
]

type PermLevel = 'loading' | 'none' | 'view' | 'edit'
type TestingPerms = Record<string, { access_mode?: string; id?: string | null }> | null
const TestingPermContext = createContext<TestingPerms>(null)

export function useTestingPerm(submodule: string): { level: PermLevel; canEdit: boolean } {
  const perms = useContext(TestingPermContext)
...
export function TestingLayout({ children }: { children: React.ReactNode }) {
  ...
  const [perms, setPerms]         = useState<TestingPerms>(null)
  ...
  useEffect(() => {
    ...
    fetch(`${API}/settings/permissions/${encodeURIComponent(user.email)}`)
      .then(r => r.json())
      .then(d => setPerms(d.permissions?.testing || {}))
      .catch(() => setPerms({}))
  }, [])
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Montserrat, sans-serif' }}>
      <div style={{ width: '220px', background: '#156082', position: 'fixed', top: 0, left: 0, height: '100vh', display: 'flex', flexDirection: 'column', zIndex: 100 }}>
        <div style={{ padding: '18px 16px 12px', borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '2px' }}>
            ...
            <span style={{ color: 'white', fontSize: '13px', fontWeight: '800', letterSpacing: '0.05em' }}>TESTING</span>
          </div>
          <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)', fontWeight: '500' }}>Test Plans, Campaigns & Remediation</div>
        </div>
      ...
      <main style={{ marginLeft: '220px', width: 'calc(100vw - 220px)', background: '#F5F7FA', minHeight: '100vh', overflowX: 'hidden' }}>
        <TestingPermContext.Provider value={perms}>
          {children}
        </TestingPermContext.Provider>
      </main>
    </div>
  )
}
