...
export function ContactArticles({ contactId }: { contactId: string }) {
  const [articles, setArticles] = useState<any[]>([])
  const [form, setForm] = useState({ title: '', url: '', description: '' })
  const [showForm, setShowForm] = useState(false)
  const [saving, setSaving] = useState(false)
  const [managingId, setManagingId] = useState<string | null>(null)
  const load = async () => setArticles(await contactsAPI.getArticles(contactId))
  useEffect(() => { load() }, [contactId])
  const handleAdd = async () => {
    if (!form.title || !form.url) return
    setSaving(true)
    try { await contactsAPI.createArticle(contactId, form); setForm({ title: '', url: '', description: '' }); setShowForm(false); load() }
          ...
          {articles.map(a => (
            <div key={a.id}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', border: '1px solid #E2E8F0', borderRadius: '8px', background: 'white' }}>
                ...
                <button onClick={() => contactsAPI.deleteArticle(contactId, a.id).then(load)} style={{ flexShrink: 0, border: 'none', background: 'none', cursor: 'pointer', color: '#9B9B9B', fontSize: '16px', lineHeight: 1 }}>×</button>
              </div>
