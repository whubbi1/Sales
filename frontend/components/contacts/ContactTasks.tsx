...
export function ContactTasks({ contactId, onChange }: { contactId: string; onChange?: () => void }) {
  const [tasks, setTasks] = useState<any[]>([])
  const [showForm, setShowForm] = useState(false)
  const [saving, setSaving] = useState(false)
  const [form, setForm] = useState({ title: '', description: '', due_date: '', priority: 'medium', status: 'todo', assigned_to: '' })
  const load = async () => setTasks(await contactsAPI.getTasks(contactId))
  useEffect(() => { load() }, [contactId])
  const handleAdd = async () => {
    if (!form.title) return
    setSaving(true)
    try { await contactsAPI.createTask(contactId, form); setForm({ title: '', description: '', due_date: '', priority: 'medium', status: 'todo', assigned_to: '' }); setShowForm(false); load(); onChange?.() }
          ...
          {tasks.map(task => {
            ...
            return (
              <div key={task.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '10px 12px', border: `1px solid ${task.status === 'done' ? '#D1FAE5' : overdue ? '#FECACA' : '#E2E8F0'}`, borderRadius: '8px', background: 'white', opacity: task.status === 'done' ? 0.65 : 1 }}>
                ...
                <button onClick={() => contactsAPI.deleteTask(contactId, task.id).then(() => { load(); onChange?.() })} style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#9B9B9B', fontSize: '16px', lineHeight: 1 }}>×</button>
              </div>
            )
          })}
        </div>
