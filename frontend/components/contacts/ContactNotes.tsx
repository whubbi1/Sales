...
export function ContactNotes({ contactId, onChange }: { contactId: string; onChange?: () => void }) {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    try { await contactsAPI.createNote(contactId, { content: newNote }); setNewNote(''); load(); onChange?.() }
          ...
          {notes.map(note => (
            <div key={note.id} style={{ background: '#FFFCF0', border: '1px solid #FDE68A', borderLeft: '4px solid #e97132', borderRadius: '8px', padding: '12px 14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <button onClick={() => contactsAPI.deleteNote(contactId, note.id).then(() => { load(); onChange?.() })} style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#9B9B9B', fontSize: '16px', marginLeft: '10px', lineHeight: 1 }}>×</button>
              </div>
