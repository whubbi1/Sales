  ...
  const leftColumn = (
    <div>
      ...
      <div style={{ background: 'white', borderRadius: '10px', border: '1px solid #EDF2F7', padding: '20px', marginBottom: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        ...
        {(company.main_erp?.length > 0 || company.cybersecurity_solutions?.length > 0 || company.grc_solutions?.length > 0 || company.sap_hosting_partner?.length > 0 || company.domain_names?.length > 0) && (
          <div style={{ marginTop: '16px', paddingTop: '14px', borderTop: '1px solid #F1F5F9', display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
            ...
            {company.cybersecurity_solutions?.length > 0 && <div><div style={{ fontSize: '9px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.07em', color: '#9B9B9B', marginBottom: '5px' }}>SAP Cybersecurity</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px' }}>{company.cybersecurity_solutions.map((v: string) => <span key={v} style={{ background: '#FFF7ED', color: '#EA580C', padding: '2px 7px', borderRadius: '10px', fontSize: '10px', fontWeight: '600' }}>{v}</span>)}</div></div>}
            {company.grc_solutions?.length > 0 && <div><div style={{ fontSize: '9px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.07em', color: '#9B9B9B', marginBottom: '5px' }}>GRC</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px' }}>{company.grc_solutions.map((v: string) => <span key={v} style={{ background: '#F5F3FF', color: '#7C3AED', padding: '2px 7px', borderRadius: '10px', fontSize: '10px', fontWeight: '600' }}>{v}</span>)}</div></div>}
