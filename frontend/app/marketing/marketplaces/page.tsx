...
function MarketplaceModal({ marketplace, onClose, onSaved }: any) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const payload = { name: name.trim(), url: url.trim(), description: description.trim() || null, created_by_email: me?.email || '' }
...
function MarketplacesContent() {
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      ...
      {showModal && <MarketplaceModal marketplace={editing} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); load() }} />}
