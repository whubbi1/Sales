...
function SoftwareContent() {
  ...
  const filtered = software.filter(s => !search || `${s.name} ${s.editor}`.toLowerCase().includes(search.toLowerCase()))
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #EDF2F7', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead style={{ background: '#FAFBFC' }}>
            <tr>
              {['Software Name', 'Editor', 'Version', 'Installation Files', 'Owner', 'Location', canEdit ? '' : null].filter(x => x !== null).map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: '48px', color: '#94A3B8' }}>No software solutions recorded yet.</td></tr>
            ) : filtered.map(item => (
