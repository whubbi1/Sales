...
export default function TicketsPage() {
  ...
  // Auto-lookup requester name when wcomply email typed
  ...
  const create = async () => {
    ...
    if (d.status === 'ok') {
      setShowModal(false)
      setForm({ title:'',description:'',category_id:'',subcategory_id:'',priority:'medium',requester_email:'',requester_name:'',requester_type:'internal',assignee_name:'',group_id:'' })
      setSubcategories([])
      router.push(`/helpdesk/tickets/${d.id}`)
    }
  }
            ...
            <div style={{ background: 'white', borderRadius: '14px', width: '100%', maxWidth: '620px', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(21,96,130,0.25)' }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div style={{ background: '#F8FAFC', borderRadius: '10px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div style={{ fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.07em', color: '#45B6E4' }}>Requester</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div>
                      <label className="form-label">Email *</label>
                      <div style={{ position: 'relative' }}>
                        <input className="form-input" type="email" value={form.requester_email} onChange={e => handleEmailChange(e.target.value)} placeholder="user@wcomply.com or client@example.com" />
                        {lookupLoading && <span style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', fontSize: '11px', color: '#45B6E4' }}>Looking up...</span>}
                      </div>
                    </div>
                    <div>
                      <label className="form-label">Name {lookupLoading && <span style={{ color: '#45B6E4' }}>auto-filling...</span>}</label>
                      <input className="form-input" value={form.requester_name} onChange={e => setForm(p => ({ ...p, requester_name: e.target.value }))} placeholder="Full name" />
                    </div>
                    <div>
                  ...
                  return (
                    <div>
                      ...
                      <select className="form-input" value={form.assignee_name} onChange={e => setForm(p => ({ ...p, assignee_name: e.target.value }))}>
                        <option value="">Auto-assign or notify group responsible</option>
                        {group.members.map((m: any) => <option key={m.user_email} value={m.user_name}>{m.user_name} {m.is_responsible ? '⭐ Responsible' : ''}</option>)}
                      </select>
                    </div>
                  )
                })()}
