    ...
    <HelpdeskLayout>
      <div style={{ padding: '24px 28px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <button onClick={() => setShowNew(true)} style={BTN.primary}>+ New Article</button>
        </div>
        ...
        {selected ? (
          <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #EDF2F7', padding: '28px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', color: '#45B6E4', fontSize: '12px', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif', fontWeight: '600', padding: 0, marginBottom: '8px', display: 'block' }}>← Back</button>
              ...
              <button onClick={() => del(selected.id)} style={BTN.danger}>Delete</button>
            </div>
            {selected.tags && <div style={{ marginBottom: '16px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>{selected.tags.split(',').map((t:string)=>t.trim()).filter(Boolean).map((t:string)=><span key={t} style={{ background: '#F1F5F9', color: '#45B6E4', padding: '2px 8px', borderRadius: '10px', fontSize: '11px' }}>#{t}</span>)}</div>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px' }}>
            ...
            : articles.map(a => (
              <div key={a.id} onClick={() => openArticle(a.id)}
                style={{ background: 'white', borderRadius: '12px', border: '1px solid #EDF2F7', padding: '20px', cursor: 'pointer', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', transition: 'all 0.15s' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#45B6E4' }}>
                  <span>{a.author_name}</span><span>👁 {a.views}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ position:'fixed',inset:0,background:'rgba(21,96,130,0.45)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'20px',backdropFilter:'blur(2px)' }} onClick={() => setShowNew(false)}>
          <div style={{ background:'white',borderRadius:'14px',width:'100%',maxWidth:'640px',maxHeight:'90vh',overflow:'auto',boxShadow:'0 20px 60px rgba(21,96,130,0.25)' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding:'18px 24px',borderBottom:'1px solid #EDF2F7',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
              <h2 style={{ fontSize:'15px',fontWeight:'800',color:'#156082',margin:0 }}>New Article</h2>
            ...
            <div style={{ padding:'20px 24px',display:'flex',flexDirection:'column',gap:'14px' }}>
              ...
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px' }}>
                ...
                <div><label className="form-label">Tags (comma separated)</label><input className="form-input" value={form.tags} onChange={e => setForm(p => ({...p,tags:e.target.value}))} placeholder="vpn, access, sap" /></div>
              </div>
              <div><label className="form-label">Content *</label><textarea className="form-input" value={form.content} onChange={e => setForm(p => ({...p,content:e.target.value}))} rows={10} placeholder="Write the article..." /></div>
            </div>
            <div style={{ padding:'14px 24px',borderTop:'1px solid #EDF2F7',display:'flex',justifyContent:'flex-end',gap:'10px',background:'#FAFBFC' }}>
              ...
              <button onClick={create} disabled={saving || !form.title || !form.content} style={{ ...BTN.primary, opacity:(saving||!form.title||!form.content)?0.6:1 }}>
