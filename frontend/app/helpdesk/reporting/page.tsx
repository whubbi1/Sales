...
export default function ReportingPage() {
  ...
  const btnStyle = (active: boolean) => ({
    padding: '7px 14px',
    borderRadius: '7px',
    border: active ? 'none' : '1.5px solid #45B6E4',
    background: active ? '#156082' : 'white',
    color: active ? 'white' : '#45B6E4',
    fontSize: '12px',
        ...
        <div style={{ padding: '28px 32px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              {[7, 30, 90].map(d => (
                <button key={d} onClick={() => setDays(d)} style={btnStyle(days === d)}>{d}d</button>
              ))}
            </div>
          </div>
