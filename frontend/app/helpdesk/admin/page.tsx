...
export default function HelpdeskAdminPage() {
  ...
  const TABS = [{id:'categories',label:'Categories',icon:'🏷️'},{id:'groups',label:'Groups',icon:'👥'},{id:'users',label:'User Roles',icon:'🔐'}]

  return (
    <HelpdeskLayout>
      <div style={{ padding:'24px 28px' }}>
        <div style={{ marginBottom:'20px' }}>
          <h1 style={{ fontSize:'20px',fontWeight:'800',color:'#156082',margin:0 }}>⚙️ Administration</h1>
        </div>
        ...
        {!loading && tab==='categories' && (
          <div style={{ display:'grid',gridTemplateColumns:'1fr 320px',gap:'20px' }}>
            <div style={{ background:'white',borderRadius:'12px',border:'1px solid #EDF2F7',overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
              {categories.map(cat=>(
                <div key={cat.id}>
                  <div style={{ padding:'12px 16px',borderBottom:'1px solid #F1F5F9',display:'flex',justifyContent:'space-between',alignItems:'center',background:'#FAFBFC' }}>
                    <div style={{ display:'flex',alignItems:'center',gap:'8px',flex:1 }}>
                      <span style={{ fontSize:'16px' }}>{cat.icon}</span>
                      <span style={{ fontSize:'13px',fontWeight:'700',color:'#156082' }}>{cat.name}</span>
                      {cat.sub_count>0&&<span style={{ fontSize:'10px',color:'#45B6E4' }}>({cat.sub_count} subs)</span>}
                    </div>
                    <div style={{ display:'flex',alignItems:'center',gap:'8px' }}>
                      <select style={{ padding:'4px 8px',borderRadius:'6px',border:'1px solid #45B6E4',fontSize:'11px',fontFamily:'Montserrat, sans-serif',minWidth:'130px' }}
                        value={cat.group_id||''} onChange={e=>assignGroup(cat.id,e.target.value)}>
                        <option value="">No group</option>
                        {groups.map((g:any)=><option key={g.id} value={g.id}>{g.name}</option>)}
                      </select>
                      <button onClick={()=>delCategory(cat.id)} style={{ ...BTN.danger,padding:'4px 8px',fontSize:'11px' }}>✕</button>
                    </div>
                  </div>
                  {cat.subcategories?.map((sub:any)=>(
                    <div key={sub.id} style={{ padding:'9px 16px 9px 40px',borderBottom:'1px solid #F1F5F9',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                      ...
                      <div style={{ display:'flex',alignItems:'center',gap:'8px' }}>
                        <select style={{ padding:'4px 8px',borderRadius:'6px',border:'1px solid #45B6E4',fontSize:'11px',fontFamily:'Montserrat, sans-serif',minWidth:'130px' }}
                          value={sub.group_id||''} onChange={e=>assignGroup(sub.id,e.target.value)}>
                          <option value="">Inherit parent</option>
                          {groups.map((g:any)=><option key={g.id} value={g.id}>{g.name}</option>)}
                        </select>
                        <button onClick={()=>delCategory(sub.id)} style={{ ...BTN.danger,padding:'4px 8px',fontSize:'11px' }}>✕</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ background:'white',borderRadius:'12px',border:'1px solid #EDF2F7',padding:'18px',boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
              <h3 style={{ fontSize:'12px',fontWeight:'700',textTransform:'uppercase',letterSpacing:'0.07em',color:'#45B6E4',marginBottom:'14px' }}>Add Category / Sub-category</h3>
              <div style={{ display:'flex',flexDirection:'column',gap:'10px' }}>
                <div><label className="form-label">Name *</label><input className="form-input" value={newCat.name} onChange={e=>setNewCat(p=>({...p,name:e.target.value}))} placeholder="Name"/></div>
                ...
                <div><label className="form-label">Parent (for sub-category)</label>
                  <select className="form-input" value={newCat.parent_id} onChange={e=>setNewCat(p=>({...p,parent_id:e.target.value}))}>
                    <option value="">None (top-level)</option>
                    {categories.map(c=><option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
                  </select>
                </div>
                <div><label className="form-label">Assign Group</label>
                  <select className="form-input" value={newCat.group_id} onChange={e=>setNewCat(p=>({...p,group_id:e.target.value}))}>
                    <option value="">No group</option>
                    {groups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}
                  </select>
                </div>
                <button onClick={createCategory} disabled={saving||!newCat.name} style={{ ...BTN.primary,opacity:(saving||!newCat.name)?0.6:1 }}>{saving?'Creating...':'+ Add'}</button>
              </div>
            </div>
          </div>
        )}
                ...
                <div key={g.id} style={{ background:'white',borderRadius:'12px',border:'1px solid #EDF2F7',padding:'18px',marginBottom:'14px',boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
                  <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'12px' }}>
                    <div>
                      ...
                      {g.responsible_name&&<p style={{ fontSize:'12px',color:'#45B6E4',margin:0 }}>⭐ {g.responsible_name}</p>}
                    </div>
                  ...
                  {g.members?.length===0?<p style={{ fontSize:'12px',color:'#45B6E4' }}>No members.</p>:(
            ...
            <div style={{ display:'flex',flexDirection:'column',gap:'14px' }}>
              <div style={{ background:'white',borderRadius:'12px',border:'1px solid #EDF2F7',padding:'18px',boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:'10px' }}>
                  ...
                  <button onClick={createGroup} disabled={saving||!newGroup.name} style={{ ...BTN.primary,opacity:(saving||!newGroup.name)?0.6:1 }}>{saving?'Creating...':'+ Create'}</button>
                </div>
              </div>
              <div style={{ background:'white',borderRadius:'12px',border:'1px solid #EDF2F7',padding:'18px',boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
                <h3 style={{ fontSize:'12px',fontWeight:'700',textTransform:'uppercase',letterSpacing:'0.07em',color:'#45B6E4',marginBottom:'14px' }}>Add Member</h3>
                <div style={{ display:'flex',flexDirection:'column',gap:'10px' }}>
                  <div><label className="form-label">Group *</label>
                    <select className="form-input" value={newMember.group_id} onChange={e=>setNewMember(p=>({...p,group_id:e.target.value}))}>
                      <option value="">Select...</option>
                      {groups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}
                    </select>
                  </div>
                  <div><label className="form-label">Email *</label><input className="form-input" value={newMember.user_email} onChange={e=>setNewMember(p=>({...p,user_email:e.target.value}))} placeholder="user@wcomply.com"/></div>
                  <div><label className="form-label">Name</label><input className="form-input" value={newMember.user_name} onChange={e=>setNewMember(p=>({...p,user_name:e.target.value}))} placeholder="Full name"/></div>
                  <label style={{ display:'flex',alignItems:'center',gap:'8px',cursor:'pointer',fontSize:'12px',color:'#45B6E4',fontWeight:'600' }}>
                    <input type="checkbox" checked={newMember.is_responsible} onChange={e=>setNewMember(p=>({...p,is_responsible:e.target.checked}))}/> ⭐ Set as responsible
                  </label>
                  <button onClick={addMember} disabled={saving||!newMember.group_id||!newMember.user_email} style={{ ...BTN.primary,opacity:(saving||!newMember.group_id||!newMember.user_email)?0.6:1 }}>{saving?'Adding...':'+ Add'}</button>
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{ background:'white',borderRadius:'12px',border:'1px solid #EDF2F7',overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
              <table style={{ width:'100%',borderCollapse:'collapse',fontSize:'12px' }}>
                <thead style={{ background:'#FAFBFC' }}>
                  <tr>{['User','Email','Role','Change'].map(h=><th key={h} style={{ padding:'10px 14px',textAlign:'left',fontSize:'10px',fontWeight:'700',textTransform:'uppercase',letterSpacing:'0.07em',color:'#45B6E4',borderBottom:'1px solid #EDF2F7' }}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {users.length===0?<tr><td colSpan={4} style={{ textAlign:'center',padding:'48px',color:'#45B6E4' }}>No users configured.</td></tr>:
                  users.map((u:any)=>{
                    const rc: Record<string,string> = {administrator:'#DC2626',helpdesk_user:'#156082',end_user:'#45B6E4'}
                    return (
                      <tr key={u.user_email} style={{ borderBottom:'1px solid #F1F5F9' }}>
                        ...
                        <td style={{ padding:'10px 14px' }}>
                          <span style={{ background:rc[u.role]+'20',color:rc[u.role],padding:'2px 8px',borderRadius:'10px',fontSize:'11px',fontWeight:'700' }}>{u.role.replace('_',' ')}</span>
                        </td>
                        <td style={{ padding:'10px 14px' }}>
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:'10px' }}>
                ...
                <div><label className="form-label">Role</label>
                  <select className="form-input" value={newUser.role} onChange={e=>setNewUser(p=>({...p,role:e.target.value}))}>
                    <option value="end_user">End User</option>
                    <option value="helpdesk_user">Helpdesk Agent</option>
                    <option value="administrator">Administrator</option>
                  </select>
                </div>
                <button onClick={upsertUser} disabled={saving||!newUser.user_email} style={{ ...BTN.primary,opacity:(saving||!newUser.user_email)?0.6:1 }}>{saving?'Saving...':'Save'}</button>
              </div>
            </div>
          </div>
        )}
