...
export default function HelpdeskPage() {
  ...
  const navItems = [
    { label: '+ Create Ticket',  href: '/helpdesk/tickets', icon: '✏️', always: true },
    { label: 'My Tickets',       href: '/helpdesk/tickets', icon: '🎫', always: true },
    { label: 'All Tickets',      href: '/helpdesk/tickets', icon: '📂', minRole: 'helpdesk_user' },
    { label: 'Assigned to Me',   href: '/helpdesk/tickets', icon: '👤', minRole: 'helpdesk_user' },
    { label: 'Knowledge Base',   href: '/helpdesk/knowledge', icon: '📚', always: true },
    { label: 'Reports',          href: '/helpdesk/reporting', icon: '📊', minRole: 'helpdesk_user' },
    { label: 'Administration',   href: '/helpdesk/admin',   icon: '⚙️', minRole: 'administrator' },
  ]
  ...
  return (
    <div style={{ display: 'flex' }}>
      ...
      <Sidebar />
      {/* Helpdesk sub-nav */}
      <div style={{ width: '190px', minHeight: '100vh', background: '#0f3a52', position: 'fixed', left: '220px', top: 0, zIndex: 90, display: 'flex', flexDirection: 'column' }}>
        ...
        <nav style={{ flex: 1, padding: '6px 8px' }}>
          {navItems.filter(i => i.always || userRole === 'administrator' || (i.minRole === 'helpdesk_user' && ['helpdesk_user','administrator'].includes(userRole))).map(item => (
            <button key={item.label} onClick={() => router.push(item.href)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', borderRadius: '6px', marginBottom: '1px', color: 'rgba(255,255,255,0.7)', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '12px', fontFamily: 'Montserrat, sans-serif', textAlign: 'left' as const }}>
              <span>{item.icon}</span>{item.label}
            </button>
          ))}
        </nav>
        <div style={{ padding: '10px 8px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <button onClick={() => router.push('/home')} style={{ width: '100%', padding: '7px 10px', borderRadius: '6px', color: 'rgba(255,255,255,0.35)', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '11px', fontFamily: 'Montserrat, sans-serif', textAlign: 'left' as const }}>
            ← All Modules
          </button>
        </div>
      </div>

      <main style={{ marginLeft: '410px', minHeight: '100vh', width: 'calc(100vw - 410px)', background: '#F5F7FA', fontFamily: 'Montserrat, sans-serif' }}>
